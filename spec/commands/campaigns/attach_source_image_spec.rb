    ...
    describe '.call' do # Changed from '#call' to '.call'
      ...
      context 'when the image is successfully downloaded and attached' do
        let(:mock_tempfile) {
          ...
          allow(mock).to receive(:content_type).and_return('image/jpeg')
          allow(mock).to receive(:original_filename).and_return('image.jpg')
          mock
        }
      ...
      context 'when the image download fails' do
        before do
          ...
          allow(Down).to receive(:download).with(source_image_url).and_raise(Down::Error, 'Download failed')
        end
      ...
      context 'when the image attachment fails' do
        ...
        before do
          ...
          allow(campaign.source_images).to receive(:attach).and_raise(StandardError, 'Attachment failed')
        end
